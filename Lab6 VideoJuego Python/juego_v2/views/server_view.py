...
from core.servidor import Servidor
...
class server_view(QtWidgets.QMainWindow, Ui_MainWindow):
    # Constructor
    def __init__(self):
        # Inicializar componentes QT5
        QtWidgets.QMainWindow.__init__(self)
        Ui_MainWindow.__init__(self)
        self.setupUi(self)
        # Se anula la página principal
        self.home_view=None
        # Se relaciona a los botones con sus correspondientes eventos
    ...
    def start_server(self):
        print('Start server')
        # Se dehabilitan controles por seguridad
        self.txt_ip.setDisabled(True)
        self.txt_port.setDisabled(True)
        self.btn_stop.setDisabled(False)
        self.btn_start.setDisabled(True)
        # Se recuperar el valor de los controles
        server_ip = self.txt_ip.toPlainText()
        server_port = self.txt_port.toPlainText()
        # Se crea instancia del servidor
        self.srv = Servidor(server_ip,int(server_port))
        # Se conecta servidor e inicia un hilo para recibir
        self.srv.conectar()
        self.srv.recibirConexion()
